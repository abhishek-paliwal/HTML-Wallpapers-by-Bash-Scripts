...
random_number=`jot -r 1  0 16}`; # finding a random number for the length of the colors array, which is 17 currently
#echo ${#primary_colors[@]};
#echo $random_number;
...
<head>
    <meta charset='utf-8'>
    <title>$random_number _ ${#primary_colors[@]} _ ${primary_colors[$random_number]} _ ${darker_primary_colors[$random_number]}</title>
    <link href='https://fonts.googleapis.com/css?family=Source+Code+Pro:400,200,300,700,900' rel='stylesheet' type='text/css'>
    <style>

    body {
        background-color: ${primary_colors[$random_number]}; /*PRIMARY COLOR*/
    }
    ...
    h1 {
        color: ${darker_primary_colors[$random_number]}; /*DARKER VERSION OF PRIMARY COLOR*/
        margin-left: 20px;
        margin-top: 20px;
        font-family: 'Source Code Pro';
        font-weight: 200;
    ...
    .custom_text_color {
        color: ${custom_text_color[$random_number]}; /* this is the text color BLACK or WHITE selected based upon the html page background.*/
    }

    mark {
        background-color: ${darker_primary_colors[$random_number]};
        color: ${primary_colors[$random_number]};
    }
